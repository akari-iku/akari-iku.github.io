...
**Cloudflareで迷ったら、「その処理はどの通り道の、どの地点に立っているか」を考えてください。**

これで大体解けます。

- **なぜこの機能はエッジで完結するのか** → 全サーバーが全サービスを動かしているから。オリジンまで戻る必要がない
- **なぜWorkersはこんなに制約が多いのか**（実行時間、メモリ、ネイティブモジュール不可） → 通り道の全地点に置けるだけ軽くないといけないから。制約は妥協ではなく前提
- **なぜCloudflare OSはエージェントにAPIキーを渡さないのか** → 渡すと処理が通り道の外に出るから。外に出たものは止められない
- **なぜ管理者が無効化しても既存Gadgetの能力は取り上げられないのか** → 通り道の制御は将来のトラフィックに効くもので、過去には遡れないから
- **どこまで繋いでいいか** → 通り道を通るものは全部見られ、記録され、止められる。逆に言えば**通り道の外に出す設計にした瞬間、Cloudflareの保証は全部切れる**。これが線引き

最後のやつが実務では一番大事だと思います。
「Cloudflareの内側に留める設計か、外に出す設計か」
この一問で、セキュリティレビューの半分くらいは片付きます。
