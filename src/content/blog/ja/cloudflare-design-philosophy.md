...
実装の決め手が「コンテナの次は何か → isolates（ブラウザのサンドボックス技術）」だったのも一貫しています。全サーバーが全サービスを動かす均質なネットワークには、重いものが乗らない。軽くないと通り道の全地点に置けないので。**アーキテクチャがランタイムの選択を決めている**わけです。

メモリ上限128MBが**有料プランに上げても引き上げられない**のも、ここから来ています。通り道に置くための条件であって、課金メニューの都合ではないので。

クラウドの制約ってだいたい金で殴れるんですけど、ここは殴れないんですよ。**思想から出ている制約は、上位プランに載らない**。これ知っておくと、上げられないものを上げようとして時間を溶かさずに済みます。

……で、ここで気になった方もいると思います。2025年6月、Cloudflareは **[Containers](https://blog.cloudflare.com/containers-are-available-in-public-beta-for-simple-global-and-programmable/)** を公開ベータで出しました。「isolatesであってコンテナではない」と言い続けてきた会社が、コンテナを入れたわけです。

前言撤回に見えますか？私も一瞬そう思ったんですが、中を見ると全然違いました。

ContainersはDurable Objectsの上に構築されていて、**その前にWorkersが立ちます**。ルーティングも認証もレート制限も、Workersが先に処理してからコンテナに渡る。つまりコンテナは通り道の**下流に置かれた**んですよ。

重い処理を載せられるようにはなったけれど、**通り道の支配は1ミリも手放していない**。やっぱり同じことをしています。
...
| Containers | 通り道の下流で重い処理を動かす（前段にWorkersが立つ） |
...
AIエージェントに認証情報を一切渡さず、外部サービスへのアクセスは1サービス1ラッパーのWorkerが仲介する。全操作をログに残し、副作用のある操作は承認キューを通す。自動フックですら例外なく通る。
...
- **WAF / DDoS防御**：なぜ専用アプライアンスを買って自社に置かなくていいのか → 通り道の入口に最初からいるから。オリジンに届く前に終わっている（機能の範囲はプランで変わりますが、機材が要らないのは共通です）
- **Workers**：なぜメモリ128MBが有料プランでも引き上げられないのか（ネイティブモジュールも不可） → 通り道の全地点に配るから。重いものは全世界には置けない。制約は妥協ではなく前提
...
- フルLinuxが要る重い処理を動かしたい → 通り道の下流に置く → **Containers**
