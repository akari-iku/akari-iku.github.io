...
実際のプロダクトでは、フロントのアーキテクチャだけを独立して選ぶことはできません。自サイトが完全静的でバックエンドを持たない、という選択は、そのまま「バックエンドを持たない」というプロダクト全体の意思決定でもありました。フロントアーキの分水嶺は、バックエンド・インフラ・チーム構成まで含めた設計全体の入り口にすぎない。この記事で開けたのは、その入り口までです。
